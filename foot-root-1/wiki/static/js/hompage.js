...
function fadein() 
{
    var leftinElements = document.querySelectorAll(".fadein");
    for (var i = 0; i < leftinElements.length; i++) {
        ...
        if (elementHeight < windowHeight / 1.5) {
            ...
            if (!leftinElements[i].classList.contains("fade-in")) {
                leftinElements[i].classList.add("fade-in");
            }
        } else {
            leftinElements[i].style.opacity = "0"; // 使元素不可见
            leftinElements[i].classList.remove("fade-in");
        }
    }
}
function leftin() 
